...
class Player:
	def __init__(self, Name=None, Race=None):
		...
		Race_path = os.path.join(Base_path, 'data/Player_race.json')
		...
		with open(Race_path, 'r') as Race_file:
			self.Race_data = json.load(Race_file)
		...
		self.atk_multi = 0
		self.speed_multi = 0
		self.evasion_multi = 0
		self.handling_multi = 0
		self.mana_multi = 0
	...
	# Player Race Mod
	def race_mod(self):
		self.health_multi = self.Race_data[f'{self.race}'['Health_multi']]
		self.atk_multi = self.Race_data[f'{self.race}'['Atk_multi']]
		self.speed_multi = self.Race_data[f'{self.race}'['Speed_multi']]
		self.evasion_multi = self.Race_data[f'{self.race}'['Evasion_multi']]
		self.handling_multi = self.Race_data[f'{self.race}'['Handling_multi']]
		self.mana_multi = self.Race_data[f'{self.race}'['Mana_multi']]
