...
class Player:
    ...
    def __init__(self, Equipment, Name=None, Race=None):
        ...
        self.skill_points = 0
        self.atk_power = 0
        self.equiped = {"Name": "Fists", "Dmg": 0} 

        # Player stats
        self.stats = {"Vit": 1, "Dex": 1, "Agi": 1, "Str": 1, "Int": 1}
        self.vit = 100
        self.dex = 5
        self.agi = 5
        self.str = 5
    ...
    def help(self):
        ...
        print("status--Shows stat values.")
    ...
    def status(self):
        ...
        for Stats, Value in self.stats.items():
            print(f"{Stats}: {Value}")
        print("---------------------------------")
        readchar.readchar()
    ...
    def attack(self, Current_enemy):
        self.atk_power = self.str + self.equiped['Dmg']
        if Current_enemy == None:
            print(f"\nYou swing your {self.equiped['Name']} like a moron at the air.")
            return None
        return "attack"
    ...
    def hud(self):
        ...
        #
        print(f"\n\n--- Race: {self.race} | {self.name} | HP: {self.vit} | Level: {self.level} | Exp: {self.exp} ---\n")
        print(f"\t\tWeapon: {self.equiped['Name']} (+{self.equiped['Dmg']} Dmg)\n  ___________________________________________________\n\n")
...
Confirmed = None
# Is user a Monster
while User.input not in ["Alnari", "Monster_Creature"]:
    User.hud()
    User.input = input("Are you a 'Alnari' or a 'Monster_Creature': ").strip().title()
Looper = True
if User.input == "Monster_Creature":
    while Looper:
        User.hud()
    ...
    while Looper:
        ...
        User.input = input("What are you: ").strip().title()
        match User.input:
